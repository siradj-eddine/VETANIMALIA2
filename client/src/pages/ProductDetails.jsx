...
export default function ProductDetails() {
  ...
  // 🔹 Spinner loader when fetching product
  if (loading) {
    return (
  ...
  return (
    <main
      className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 overflow-x-hidden"
    ...
    >
      ...
      <div className="mt-8 flex flex-col lg:flex-row gap-8" key={product._id}>
        {/* LEFT */}
        <div className="flex flex-col md:flex-row gap-6 w-full lg:w-1/2 items-center justify-center md:translate-x-[-20px] min-lg:translate-x-[70px]">
          {/* Thumbnails */}
          <div className="flex gap-4 md:flex-col order-2 md:order-1 max-md:justify-center">
            {product?.image?.map((img, idx) => (
              <button
                ...
                aria-label={t("productDetails.alt.thumb")}
                title={t("productDetails.alt.thumb")}
              >
                <img
                  src={img.url}
                  alt={t("productDetails.alt.thumb")}
          ...
          <div className="flex items-center justify-center order-1 md:order-2 flex-1 md:mr-25 md:max-w-fit">
            <img
              ...
              alt={product?.name || t("productDetails.alt.main")}
        ...
        <div className="w-full lg:w-1/2 flex flex-col md:flex-row gap-6 items-center">
          <div className="space-y-3 text-sm w-full md:w-1/2 max-md:ml-[20%] overflow-x-hidden">
            ...
            <span className="flex">
              <p className="font-bold mr-1">{t("productDetails.id")} :</p>
              {product._id}
            </span>
          </div>
          ...
          <div className="w-full md:w-1/2">
            <img
              src={catImage}
              alt={t("productDetails.alt.cat")}
              height={80}
              width={80}
              className="ml-[35%] z-0 max-lg:hidden -mt-28 translate-y-4"
            />
            <div className="rounded-2xl border-2 border-[var(--mainOrange)] p-5 sticky top-4 min-lg:bg-white z-10 ">
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight w-full text-center">
                {t("productDetails.price")} : {product?.price * quantity} DZD
              </h1>
      ...
      <section>
        <h1 className="font-semibold text-3xl max-sm:text-center">
